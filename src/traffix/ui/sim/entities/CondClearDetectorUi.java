...
public class CondClearDetectorUi extends UiEntity {
  private CondClearDetector m_detector;

  CondClearDetectorUi(IUiEntityContextProvider cp, CondClearDetector d) {
    super(cp);
    m_detector = d;
  }

  public UiEntityType getType() {
    return UiEntityType.CONDCLEARDETECTOR;
  }
  ...
  public void paint() {
    ...
    if (!isInEditMode()) {
      Color bg = Colors.get(Config.RGB_CONDCLEAR_DETECTOR_INACTIVE);
      if (m_detector.getState()==CondClearDetector.S_PRESSED)
        bg = Colors.get(Config.RGB_CONDCLEAR_DETECTOR_ACTIVE);
      gc.setBackground(bg);
